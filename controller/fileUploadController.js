...
const studentFilesUpload = async (req) => {
  ...
  try {
    ...
    if (!user_id || !applicant_id) {
      throw new Error(
        "User ID and Applicant ID are required for file uploads."
      );
    }
...
const getSecureFiles = async (req, res) => {
  try {
    ...
    const SERVER_URL = "http://192.168.1.11:5000/uploads/";

    // Build file URLs, ensuring no duplicate 'uploads/'
...
const uploadFacultyFiles = async (req, res) => {
  try {
    ...
    if (!profilePicturePath) {
      return res
        .status(400)
        .json({ error: "Failed to process the profile picture." });
    }
...
const getFacultyFiles = async (req, res) => {
  try {
    ...
    if (profilePicturePath && fs.existsSync(profilePicturePath)) {
      files.profile_picture_url = `/uploads/facultyPhoto/${path.basename(
        profilePicturePath
      )}`;
    }
    ...
    if (Object.keys(files).length === 0) {
      return res
        .status(404)
        .json({
          error: "No files exist on the server for the given faculty member",
        });
    }
...
module.exports = {
  studentFilesUpload,
  /* getFilesByUserId,*/ getSecureFiles,
  uploadFacultyFiles,
  getFacultyFiles,
};
